...
import { BrandDesktopTable, BrandMobileList, BrandIndexPageProps } from '@/features/brand';
import { Input } from '@/components/ui/input';
import { useState, useCallback } from 'react';
import { debounce } from 'lodash';
import { useTranslation } from 'react-i18next';

export default function BrandIndexPage({ brands, state }: BrandIndexPageProps & { state?: { search?: string, sort?: string } }) {
                    ...
                    <Card>
                        ...
                        <CardBody className="px-0">
                            <div className="hidden md:block">
                                <BrandDesktopTable brands={brands.data} state={state} />
                            </div>

                            <div className="block md:hidden">
                                <BrandMobileList brands={brands.data} />
                            </div>
                        </CardBody>
...
const IndexLayout = ({ children }: { children: React.ReactNode }) => {
    const { t } = useTranslation();
    return (
        <AuthenticatedLayout header={<Heading size="sm">{t('brands.title')}</Heading>}>
            {children}
        </AuthenticatedLayout>
    );
};

BrandIndexPage.layout = (page: any) => <IndexLayout>{page}</IndexLayout>;
