  ...
  return (
    ...
    >
      {(value: string | SelectItem | null) => (
        <SelectRenderValue value={value} placeholder={placeholder} options={options} />
      )}
    </BaseSelect.Value>
  );
}

export type { SelectItem as SelectItemType };
...
function SelectRenderValue({
  ...
  if (options) {
    ...
    if (matched) {
      return <span className="text-popover-foreground select-none">{matched.label}</span>;
    }
  }
